...
module use /home/groups/s-ees/share/sdss/spack_cees/spack/share/spack/lmod_opensees/linux-centos7-x86_64/Core
...
# NOTE: should probably pick a version and pull that tag in the git pull, but this is probably from the devel/main branch
# and the most recent version is (probably 3.5.0), so this is really 3.5.0+, which we'll call 3.5.1.
# *** PRODUCTION TARGET:
#TARGET_PATH="/home/groups/s-ees/share/sdss/software/no_arch/gcc/12.1.0/opensees/3.5.1"

if [ ! -d OpenSees ]; then
  git clone git@github.com:OpenSees/OpenSees.git
fi
cd OpenSees
# TODO:? This would be the place to pick a version...
#
if [ -d build ]; then
  rm -rf build
fi
...
#
CFLAGS=$CFLAGS CXXFLAGS=$CXXFLAGS LDFLAGS=$LDFLAGS MPI_HOME=${MPI_ROOT} cmake ../ ${CMAKE_FLAGS} -DCMAKE_PREFIX_PATH="${MPI_ROOT}" -DCMAKE_CXX_COMPILER=${MPI_ROOT}/bin/mpic++ -DCMAKE_C_COMPILER=$MPICC -DCMAKE_Fortran_COMPILER=${MPIF90} -DBLA_STATIC=ON -DMKL_LINK=static -DCMAKE_INSTALL_PREFIX=${TARGET_PATH}
make -j 1
make -j 1 install
